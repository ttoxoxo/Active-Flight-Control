/*
  ...
  This example shows how to obtain RTCM data from a NTRIP Caster over WiFi and push it over Serial to a ZED-F9x.
  ...
  5V / V_USB -> ZED-F9P Breakout 5V
  Serial1 TX (3.3V) -> ZED-F9P UART1 RX
  Serial1 RX (3.3V) -> ZED-F9P UART1 TX
  GND -> GND
...
bool processConnection()
{
  if (ntripClient.connected() == true) // Check that the connection is still open
  {
    ...
    if (rtcmCount > 0)
    {
      ...
      //Push RTCM to GNSS module over Serial
      if (myGNSS.pushRawData(rtcmData, rtcmCount))
      {
